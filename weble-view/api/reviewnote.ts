...
export default async function handler(req: AnyReq, res: AnyRes) {
  ...
  if (method === "OPTIONS") {
    ...
    res.setHeader(
      ...
      "authorization,content-type,accept,x-reviewnote-cookie"
    );
  ...
  const targetUrl = `https://www.reviewnote.co.kr${pathname}${sp.toString() ? `?${sp.toString()}` : ""}`;

  const headersIn = req.headers ?? {};
  const cookieRaw = headersIn["x-reviewnote-cookie"];
  ...
  const upstream = await fetch(targetUrl, {
    ...
    headers: {
      ...
      referer: "https://www.reviewnote.co.kr/campaigns?channel=BLOG&sort=DELIVERY"
    }
  }).catch((e) => {
