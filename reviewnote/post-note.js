const { chromium } = require("playwright");
const { pickFirstVisibleLocator, debugDump } = require("../core/utils");

const WRITE_URL = "https://www.reviewnote.co.kr/communities/friend/new";

(async () => {
  const [,, title, content, category] = process.argv;

  if (!title || !content || !category) {
    console.error('Usage: node post.js "제목" "본문" "카테고리(블로그|인스타그램 또는 BLOG|INSTAGRAM)"');
    process.exit(1);
  }

  const categoryMap = {
    "블로그": "BLOG",
    "인스타그램": "INSTAGRAM",
    "BLOG": "BLOG",
    "INSTAGRAM": "INSTAGRAM",
  };
  const categoryValue = categoryMap[category];
  if (!categoryValue) {
    console.error(`Invalid category: "${category}" (allowed: 블로그, 인스타그램, BLOG, INSTAGRAM)`);
    process.exit(1);
  }

  const browser = await chromium.launch({ headless: false });
  const context = await browser.newContext({ storageState: "storageState.json" });
  const page = await context.newPage();

  await page.goto(WRITE_URL, { waitUntil: "networkidle" });
  await page.waitForTimeout(1500);

  // 리다이렉트 감지: 홈이나 다른 페이지로 튕겼는지 확인
  const currentUrl = page.url();
  if (!currentUrl.includes("/communities/friend/new")) {
    console.error(`[reviewnote] 글쓰기 페이지로 이동 실패. 현재 URL: ${currentUrl}`);
    console.error(
      [
        "  → 세션이 만료되었을 수 있습니다.",
        "  → 아래 명령 1개 실행 후 다시 시도하세요:",
        "  npm run login-note",
      ].join("\n"),
    );
    await browser.close();
    process.exit(1);
  }

  // 1) 제목 입력
  const titleLocator = await pickFirstVisibleLocator(page, [
    page.locator('input[name="title"]'),
    page.locator('input[placeholder*="제목"]'),
    page.getByRole("textbox", { name: /제목/ }),
  ]);
  if (!titleLocator) {
    console.error("[reviewnote] 제목 입력칸을 찾지 못했습니다.");
    await debugDump(page);
    await browser.close();
    process.exit(1);
  }
  await titleLocator.fill(title);

  // 2) 본문 입력 (Toast UI Editor: ProseMirror)
  const bodyLocator = await pickFirstVisibleLocator(page, [
    page.locator("div.ProseMirror.toastui-editor-contents"),
    page.locator("div.ProseMirror"),
    page.locator('[contenteditable="true"]'),
  ]);
  if (!bodyLocator) {
    console.error("[reviewnote] 본문 입력칸(에디터)을 찾지 못했습니다.");
    await debugDump(page);
    await browser.close();
    process.exit(1);
  }
  await bodyLocator.click({ force: true });
  await page.keyboard.down("Meta");
  await page.keyboard.press("KeyA");
  await page.keyboard.up("Meta");
  await page.keyboard.press("Backspace");
  await page.keyboard.type(content, { delay: 5 });

  // 2.5) 카테고리 선택
  const select = page.locator('select[name="category"]');
  if ((await select.count()) === 0) {
    console.error('[reviewnote] 카테고리 select를 찾지 못했습니다.');
    await debugDump(page);
    await browser.close();
    process.exit(1);
  }
  await select.selectOption({ value: categoryValue });
  const selected = await select.inputValue();
  if (!selected) {
    console.error(`[reviewnote] 카테고리 선택 실패: "${categoryValue}"`);
    await debugDump(page);
    await browser.close();
    process.exit(1);
  }
  await page.waitForTimeout(200);

  // 3) 등록 버튼 클릭
  const submit = await pickFirstVisibleLocator(page, [
    page.getByRole("button", { name: /등록|작성|올리기/ }),
    page.locator('button:has-text("등록")'),
    page.locator('button:has-text("작성")'),
  ]);
  if (!submit) {
    console.error("[reviewnote] 등록 버튼을 찾지 못했습니다.");
    await debugDump(page);
    await browser.close();
    process.exit(1);
  }
  await submit.waitFor({ state: "visible", timeout: 10000 }).catch(() => {});
  await submit.click({ force: true });
  await page.waitForTimeout(2000);

  await browser.close();
  console.log("[reviewnote] 글 작성 완료!");
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
